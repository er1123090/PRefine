"""Experiment4 prompt compatibility surface.

The vanilla prompt is imported from the preserved experiment4 file.  The two
memory prompts are the shared prompt block used by the experiment4 inference
scripts; the original standalone ``prompt_inference.py`` is no longer present
in experiment4, so the block is kept here as an explicit compatibility copy.
"""

from methods.vanilla_llm.prompt import IMPLICIT_ZS_PROMPT_TEMPLATE

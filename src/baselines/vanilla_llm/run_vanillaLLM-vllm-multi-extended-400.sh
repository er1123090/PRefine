...
# 1. Environment & Paths
# ==============================================================================
PYTHON_SCRIPT="/data/minseo/experiments6/vanillaLLM/vanillaLLM_inference-vllm-multi.py"

INPUT_PATH="/data/minseo/experiments6/data/1229_dev_6.json"
MULTITURN_PATH="/data/minseo/experiments6/query_new_multi.json"
...
BASE_OUTPUT_DIR="/data/minseo/experiments6/vanillaLLM/inference/extended/outputs/multiturn/vllm"
BASE_LOG_DIR="/data/minseo/experiments6/vanillaLLM/inference/extended/logs/multiturn/vllm"
...
wait_for_server() {
    ...
    while ! curl -s "$VLLM_URL/models" > /dev/null; do
        ...
        if ! ps -p "$SERVER_PID" > /dev/null; then
            ...
            cat vllm_server_multi_extended.log
            exit 1
        fi
...
# ==============================================================================
echo "========================================================"
echo "VanillaLLM Extended vLLM Multi-turn Started at $(date)"
echo "Query File : $MULTITURN_PATH"
...
for model in "${MODELS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID nohup vllm serve "$model" \
        ...
        --trust-remote-code > vllm_server_multi_extended.log 2>&1 &
...
echo "========================================================"
echo "VanillaLLM Extended vLLM Multi-turn Finished at $(date)"
echo "========================================================"

...
class PreferenceAggregator:
    ...
    async def update_memory(self, current_state: MemoryState,
        ...
        if self.memory_mode == "generate_verify":
            ...
            if candidate_pref_json:
                candidate_pref = json.dumps(
                    candidate_pref_json, ensure_ascii=False, indent=2
                )
                is_valid, new_feedback, verifier_input, verifier_output_json = (
                    await self._verify_preference(
                        prev_implicit=current_state.implicit_pref,
                        full_dialogue=prompt_dialogue,
                        full_api_calls=full_api_str,
                ...
                session_attempts_log.append(
                    {
                        ...
                        "stage": "generate_then_verify_no_refine",
            ...
            updated_log.append(
                {
                    ...
                    "final_preference_at_session": (
                        ...
                        if isinstance(candidate_pref, str)
                        and candidate_pref not in ["None", "{}"]
                        else {}
                    ),
        ...
        try:
            for session in sessions:
                ...
                current_state = await aggregator.update_memory(
                    current_state, dialogue_text, api_calls
                )
        finally:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", default=os.path.join(_ROOT, "data", "MPT_v2_mix600.json")
    )
    ...
    parser.add_argument("--memory_mode",
                        ...
                        help=(
                            ...
                            "generate_verify runs G+V once without a refinement step; "

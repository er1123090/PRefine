...
def audit(
    ...
    _check(runtime["model_id"] == model_id, "runtime model identity is pinned", evidence)
    _check(
        runtime["tool_call_parser"] == tool_call_parser and runtime.get("reasoning_parser") == reasoning_parser,
        "runtime tool/reasoning parsers match the model contract",
        evidence,
    )
    _check(
        runtime["enable_auto_tool_choice"] is True and runtime["thinking"] is thinking,
        "auto tool choice and thinking policy match the model contract",
        evidence,
    )
    _check(runtime.get("reasoning_effort") == reasoning_effort, "reasoning effort matches the model contract", evidence)
...
def main() -> None:
    ...
    result = audit(
        run_root, args.model_id, args.tool_call_parser, args.reasoning_parser, args.thinking, args.reasoning_effort
    )

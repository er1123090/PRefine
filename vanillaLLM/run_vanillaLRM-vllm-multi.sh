...
# [중요] 실행할 Python 스크립트 경로 (저장한 파이썬 파일명으로 정확히 변경하세요)
PYTHON_SCRIPT="/data/minseo/experiments4/vanillaLLM/vanillaLRM_inference-vllm-multi2.py"

# 데이터 및 스키마 경로
INPUT_PATH="/data/minseo/experiments4/data/1229_dev_6.json"
# Python 코드에서 --multiturn_path로 받을 파일입니다.
QUERY_PATH="/data/minseo/experiments4/query_multiturn-domain.json" 
...
# 출력 및 로그 디렉토리
BASE_OUTPUT_DIR="/data/minseo/experiments4/vanillaLLM/inference/1230-1_output_multiturn"
BASE_LOG_DIR="/data/minseo/experiments4/vanillaLLM/inference/1230-1_logs_multiturn"

# 태그 설정
DATE_TAG="$(date +%m%d)"
TEST_TAG="test_deepseek2"
    ...
    for prompt_type in "${PROMPT_TYPES[@]}"; do
        for context in "${CONTEXT_TYPES[@]}"; do
            for pref in "${PREF_TYPES[@]}"; do
                ...
                # 출력 경로 생성
                OUTPUT_DIR="$BASE_OUTPUT_DIR/$context/$pref/multiturn-query/$MODEL_SAFE_NAME/$prompt_type"
                LOG_DIR="$BASE_LOG_DIR/$context/$pref/multiturn-query/$MODEL_SAFE_NAME/$prompt_type"
                ...
                # [수정됨] Python 실행: --query_path 대신 --multiturn_path 사용

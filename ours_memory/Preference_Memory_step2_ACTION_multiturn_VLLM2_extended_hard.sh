...
for INF_MODEL in "${INFERENCE_MODELS[@]}"; do
    ...
    for MEM_FOLDER in "${MEMORY_FOLDERS[@]}"; do
        ...
        BASE_OUTPUT_DIR="/data/minseo/experiments4/ours_memory/inference/extended/outputs/multiturn/vllm/${MEM_FOLDER}"
        BASE_LOG_DIR="/data/minseo/experiments4/ours_memory/inference/extended/logs/multiturn/vllm/${MEM_FOLDER}"
        ...
        for context in "${CONTEXT_TYPES[@]}"; do
            for pref in "${PREF_TYPES[@]}"; do
                ...
                DATE_TAG="$(date +%m%d)"
                CURRENT_OUT_DIR="$BASE_OUTPUT_DIR/$context/$pref/${INF_SAFE_NAME}_high/$PROMPT_NAME"
                CURRENT_LOG_DIR="$BASE_LOG_DIR/$context/$pref/${INF_SAFE_NAME}_high/$PROMPT_NAME"

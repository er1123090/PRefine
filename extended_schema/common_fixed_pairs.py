...
def get_multiturn_utterance(module: Any, item: Dict[str, Any]) -> str:
    ...
    query_turns = item.get("query") or []
    if isinstance(query_turns, list):
        return format_multiturn_dialogue(query_turns)
...
def build_multiturn_ground_truth(
    ...
    _domain, base_args = parse_api_call_to_dict(str(api_calls[0]))
    ...
    return merge_and_generate_api_strings(domain, base_args, normalized_slot_values)

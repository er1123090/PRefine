...
ROOT = Path("/data/minseo/experiments5/extended_schema/mem0")
...
SINGLE_SOURCE = Path("/data/minseo/experiments5/methods/mem0/inference_api_singleturn.py")
MULTI_SOURCE = Path("/data/minseo/experiments5/methods/mem0/inference_api_multiturn.py")

SINGLE_SCHEMA_PATH = Path("/data/minseo/experiments5/config/schema_easy_extended.json")
MULTI_SCHEMA_PATH = Path("/data/minseo/experiments5/config/schema_all_extended_complete.json")
...
async def rerun_singleturn_row(
    ...
    async with semaphore:
        ...
        if llm_result.get("error"):
            ...
            updated["llm_output"] = llm_result["output"]
            updated["reasoning_content"] = llm_result["reasoning_content"]
...
async def rerun_multiturn_row(
    ...
    async with semaphore:
        ...
        prompt = module.build_mem0_input_prompt(
            ...
            retrieved_memories=updated.get("retrieved_memories") or [],
            current_user_utterance=str(updated.get("test_utterance") or ""),
            template=module.IMPLICIT_ZS_PROMPT_MEMORY_TEMPLATE_MULTITURN,
            context_type=CONTEXT_TYPE,
            tools_schema=tools_schema,
        )
        llm_result = await call_with_retries(module, prompt, tools_schema)

...
EXTENDED_ROOT = Path("/data/minseo/experiments5/extended_schema")
THIS_DIR = Path(__file__).resolve().parent
EVAL_ROOT = Path("/data/minseo/experiments5/methods/our_memory/session_memory_eval_0312")
PREF_LIST_PATH = Path("/data/minseo/experiments5/config/pref_list_extended.json")
...
def parse_single_meta(method: str, task_root: Path, json_path: Path) -> Dict[str, str]:
    ...
    if method == "vanilla_llm":
        if len(rel_parts) != 6:
            raise ValueError(f"Unexpected vanilla singleturn layout: {json_path}")
        context_type, pref_type, query_variant, action_model_raw, prompt_name, file_name = rel_parts
        memory_model = ""
    elif method in {"langmem", "ours_memory"}:
        if len(rel_parts) != 6:
            raise ValueError(f"Unexpected {method} singleturn layout: {json_path}")
        memory_model, context_type, pref_type, action_model_raw, prompt_name, file_name = rel_parts
        query_variant = "singleturn"
    elif method == "mem0":
        if len(rel_parts) != 5:
            raise ValueError(f"Unexpected mem0 singleturn layout: {json_path}")
        context_type, pref_type, action_model_raw, prompt_name, file_name = rel_parts
        memory_model = ""
        query_variant = "singleturn"
    else:
...
def parse_multi_meta(method: str, task_root: Path, json_path: Path) -> Dict[str, str]:
    ...
    if method == "vanilla_llm":
        if len(rel_parts) != 6:
            raise ValueError(f"Unexpected vanilla multiturn layout: {json_path}")
        context_type, pref_type, query_variant, action_model_raw, prompt_name, file_name = rel_parts
        memory_model = ""
    elif method in {"langmem", "ours_memory"}:
        if len(rel_parts) != 6:
...
def build_singleturn_rows() -> List[Dict[str, Any]]:
    configs = [
        ("vanilla_llm", EXTENDED_ROOT / "vanilla_llm" / "output" / "singleturn" / "api"),
        ("mem0", EXTENDED_ROOT / "mem0" / "output" / "singleturn" / "api"),
        ("langmem", EXTENDED_ROOT / "langmem" / "output" / "singleturn" / "api"),
        ("ours_memory", EXTENDED_ROOT / "ours_memory" / "output" / "singleturn" / "api"),
    ]
    rows: List[Dict[str, Any]] = []
...
def build_multiturn_rows() -> List[Dict[str, Any]]:
    ...
    configs = [
        ("vanilla_llm", EXTENDED_ROOT / "vanilla_llm" / "output" / "multiturn" / "api"),
        ("mem0", EXTENDED_ROOT / "mem0" / "output" / "multiturn" / "api"),
        ("langmem", EXTENDED_ROOT / "langmem" / "output" / "multiturn" / "api"),
        ("ours_memory", EXTENDED_ROOT / "ours_memory" / "output" / "multiturn" / "api"),
    ]
    rows: List[Dict[str, Any]] = []
    for method, task_root in configs:
...
def main() -> None:
    ...
    single_csv = THIS_DIR / "singleturn_comparison.csv"
    multi_csv = THIS_DIR / "multiturn_comparison.csv"

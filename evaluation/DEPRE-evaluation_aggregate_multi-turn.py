...
def parse_api_string(api_string: Any) -> List[Tuple[str, Dict[str, str]]]:
    """
    Parse:
      - "GetX(a=1) GetY(b='c')"  (regex)
      - JSON dict (fallback): {"slot": "..."} -> ("__JSON_PARSED__", dict)
    Returns list of (func_name, slot_dict).
    """
    if api_string is None:
        return []

    # If it's already a dict, treat as JSON-parsed
    if isinstance(api_string, dict):
        return [("__JSON_PARSED__", {str(k): normalize_value(v) for k, v in api_string.items()})]
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            return parsed_calls
        # If list, ignore (ambiguous) unless you want custom logic
    except Exception:
        pass
...
def main():
    ...
    parser.add_argument("--pref_file", type=str, default="/data/minseo/experiments5/config/pref_list.json", help="Preference map json (optional)")

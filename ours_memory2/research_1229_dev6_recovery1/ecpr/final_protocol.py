...
def _compute_baseline_contract(value: Any, label: str) -> None:
    if not isinstance(value, dict) or set(value) != {
        "ownership_contract",
        "capture_scope",
        "compute_apps",
        ...
        raise ValueError(f"{label} compute baseline field contract violation")
    expected = value.get("expected_gpus")
    if (
        value.get("ownership_contract") != GPU_OWNERSHIP_CONTRACT
        or value.get("capture_scope") != "all_nvidia_compute_apps"
        or value.get("compute_apps") != []
        or not isinstance(expected, list)
        or len(expected) != 4
        or any(
            not isinstance(item, dict)
            or set(item) != {"gpu_uuid", "pci_bus_id"}
            or not isinstance(item.get("gpu_uuid"), str)
            or not item["gpu_uuid"]
            or not isinstance(item.get("pci_bus_id"), str)
            or not item["pci_bus_id"]
            for item in expected
        )
        or len({item["gpu_uuid"] for item in expected}) != 4
        or len({item["pci_bus_id"] for item in expected}) != 4
    ):
        raise ValueError(f"{label} compute baseline value contract violation")
...
def _topology_contract(value: Any, label: str) -> None:
    if not isinstance(value, dict) or set(value) != {
        ...
        "mapped_gpu_uuids",
        "prelaunch_compute_apps_sha256",
    }:
        raise ValueError(f"{label} topology field contract violation")
    if (
        value.get("ownership_contract") != GPU_OWNERSHIP_CONTRACT
        or value.get("pid_namespace_contract") != PID_NAMESPACE_CONTRACT
    ):
        raise ValueError(f"{label} topology ownership contract violation")
    _require_digest(
        value.get("prelaunch_compute_apps_sha256"),
        f"{label} prelaunch compute baseline digest",
    )
    ...
    if (
        ...
        or any(
            ...
            or not isinstance(mapping.get("host_pid"), int)
            or mapping["host_pid"] <= 0
            or not isinstance(mapping.get("gpu_uuid"), str)
            or not mapping["gpu_uuid"]
            or not isinstance(mapping.get("pci_bus_id"), str)
            or not mapping["pci_bus_id"]
            or not isinstance(mapping.get("used_gpu_memory_mib"), int)
            or mapping["used_gpu_memory_mib"] <= 0
    ...
    if (
        len(mappings) != 4
        or len({mapping["host_pid"] for mapping in mappings}) != 4
        or len({mapping["gpu_uuid"] for mapping in mappings}) != 4
        or len({mapping["pci_bus_id"] for mapping in mappings}) != 4
    ):
        raise ValueError(f"{label} compute-app mappings are not four-way unique")
...
def _runtime_contract(payload: Any) -> None:
    ...
    _compute_baseline_contract(
        vllm.get("prelaunch_compute_apps"), "runtime prelaunch"
    )
    ...
    expected_compute_gpus = [
        {"gpu_uuid": item["uuid"], "pci_bus_id": item["pci_bus_id"]}
        for item in gpus
    ]
    baseline = vllm["prelaunch_compute_apps"]
    topology = vllm["topology_at_readiness"]
    if baseline["expected_gpus"] != expected_compute_gpus:
        raise ValueError("runtime compute baseline differs from GPU identities")
    baseline_sha256 = hashlib.sha256(
        canonical_json(baseline).encode("utf-8")
    ).hexdigest()
    if topology["prelaunch_compute_apps_sha256"] != baseline_sha256:
        raise ValueError("runtime topology does not bind the empty compute baseline")
    expected_bus_ids = {
        item["uuid"]: item["pci_bus_id"]
        for item in gpus
    }
    if {
        mapping["gpu_uuid"]: mapping["pci_bus_id"]
        for mapping in topology["compute_app_mappings"]
    } != expected_bus_ids:
        raise ValueError("runtime compute apps differ from GPU UUID/PCI identities")
...
def validate_stage_chain(root: Path) -> dict[str, dict[str, Any]]:
    ...
    if runtime is not None:
        ...
        runtime_baseline = runtime_payload["vllm"]["prelaunch_compute_apps"]
        runtime_baseline_sha256 = hashlib.sha256(
            canonical_json(runtime_baseline).encode("utf-8")
        ).hexdigest()
        expected_gpu_uuids = sorted(gpu["uuid"] for gpu in runtime_payload["gpus"])
        expected_gpu_bus_ids = {
            gpu["uuid"]: gpu["pci_bus_id"] for gpu in runtime_payload["gpus"]
        }
        ...
        if (
            runtime_topology["prelaunch_compute_apps_sha256"]
            != runtime_baseline_sha256
            or {
                mapping["gpu_uuid"]: mapping["pci_bus_id"]
                for mapping in runtime_topology["compute_app_mappings"]
            }
            != expected_gpu_bus_ids
        ):
            raise ValueError("runtime topology differs from the exclusive GPU lease")

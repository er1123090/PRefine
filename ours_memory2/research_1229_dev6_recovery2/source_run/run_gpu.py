...
def runtime_precheck(_root: Path, contract: dict[str, Any]) -> tuple[bool, str]:
    ...
    try:
        environment = sanitized_child_environment(
            _root, contract["runtime"]["gpu_indices"]
        )
        gpus = _gpu_identities(environment)
        baseline = capture_empty_compute_baseline(environment, gpus)
    except (OSError, RuntimeError, ValueError) as exc:
        return False, f"exclusive GPU baseline failed: {type(exc).__name__}: {exc}"
    return True, canonical_json(
        {
            "local_model_snapshot": True,
            "vllm_executable": True,
            "gpu_compute_baseline": baseline,
        }
    )
...
def _compute_app_rows(environment: dict[str, str]) -> list[dict[str, Any]]:
    output = _capture_text(
        [
            "nvidia-smi",
            "--query-compute-apps=pid,gpu_uuid,gpu_bus_id,used_gpu_memory",
            "--format=csv,noheader,nounits",
        ],
        environment,
    )
    ...
    for line in output.splitlines():
        pieces = [piece.strip() for piece in line.split(",")]
        if len(pieces) != 4 or not pieces[0].isdigit() or not pieces[3].isdigit():
            raise ValueError("malformed nvidia-smi compute-app row")
        host_pid = int(pieces[0])
        used_gpu_memory_mib = int(pieces[3])
        if host_pid <= 0 or not pieces[1] or not pieces[2]:
            raise ValueError("invalid nvidia-smi compute-app identity")
        rows.append(
            {
                "host_pid": host_pid,
                "gpu_uuid": pieces[1],
    ...
    if len(
        {
            (row["host_pid"], row["gpu_uuid"], row["pci_bus_id"])
            for row in rows
        }
    ) != len(rows):
        raise ValueError("duplicate nvidia-smi compute-app row")
    return rows


def _expected_compute_gpus(gpus: list[dict[str, Any]]) -> list[dict[str, str]]:
    expected = [
        {"gpu_uuid": gpu["uuid"], "pci_bus_id": gpu["pci_bus_id"]}
        for gpu in gpus
    ]
    if (
        len(expected) != 4
        or len({row["gpu_uuid"] for row in expected}) != 4
        or len({row["pci_bus_id"] for row in expected}) != 4
    ):
        raise ValueError("exclusive GPU lease requires four unique GPU identities")
    return expected


def capture_empty_compute_baseline(
    environment: dict[str, str], gpus: list[dict[str, Any]]
) -> dict[str, Any]:
    rows = _compute_app_rows(environment)
    if rows:
        raise ValueError("sealed GPUs already have active compute applications")
    return {
        "ownership_contract": GPU_OWNERSHIP_CONTRACT,
        "capture_scope": "all_nvidia_compute_apps",
        "compute_apps": [],
        "expected_gpus": _expected_compute_gpus(gpus),
    }


def validate_empty_compute_baseline(
    baseline: dict[str, Any], gpus: list[dict[str, Any]]
) -> None:
    expected = {
        "ownership_contract": GPU_OWNERSHIP_CONTRACT,
        "capture_scope": "all_nvidia_compute_apps",
    ...
    if baseline != expected:
        raise ValueError("prelaunch compute-app baseline contract mismatch")


def validate_exclusive_compute_apps(
    rows: list[dict[str, Any]], gpus: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    expected = {gpu["uuid"]: gpu["pci_bus_id"] for gpu in gpus}
    if len(expected) != 4 or len(rows) != 4:
        raise ValueError("exclusive runtime must expose exactly four compute apps")
    if len({row["host_pid"] for row in rows}) != 4:
        raise ValueError("exclusive runtime compute-app host PIDs are not unique")
    if len({row["gpu_uuid"] for row in rows}) != 4:
        raise ValueError("exclusive runtime GPU UUIDs are not unique")
    if {row["gpu_uuid"] for row in rows} != set(expected):
        raise ValueError("exclusive runtime compute apps differ from sealed GPU UUIDs")
    for row in rows:
        if row["pci_bus_id"] != expected[row["gpu_uuid"]]:
            raise ValueError("exclusive runtime GPU UUID/PCI identity mismatch")
        if row["used_gpu_memory_mib"] <= 0:
            raise ValueError("exclusive runtime compute app has no GPU memory")
    return rows
...
def capture_server_topology(
    ...
    prelaunch_compute_apps: dict[str, Any],
) -> dict[str, Any]:
    validate_empty_compute_baseline(prelaunch_compute_apps, gpus)
    process_group = os.getpgid(process.pid)
    descendants = descendant_process_identities(process.pid)
    mappings = validate_exclusive_compute_apps(_compute_app_rows(environment), gpus)
    mapped_uuids = {mapping["gpu_uuid"] for mapping in mappings}
    return {
        "ownership_contract": GPU_OWNERSHIP_CONTRACT,
        "pid_namespace_contract": PID_NAMESPACE_CONTRACT,
        ...
        "prelaunch_compute_apps_sha256": sha256_bytes(
            canonical_json(prelaunch_compute_apps).encode("utf-8")
        ),
    }
...
def build_runtime_payload(
    ...
    payload = {
        ...
        "vllm": {
            ...
            "prelaunch_compute_apps": prelaunch_compute_apps,
            "topology_at_readiness": capture_server_topology(
                process, environment, gpus, prelaunch_compute_apps
            ),
        },
        "model": {
...
def validate_runtime_payload(
    ...
    *,
    ...
    baseline = payload["vllm"]["prelaunch_compute_apps"]
    validate_empty_compute_baseline(baseline, payload["gpus"])
    current_topology = capture_server_topology(
        process, environment, payload["gpus"], baseline
    )
    topology_identity = [
        (row["host_pid"], row["gpu_uuid"], row["pci_bus_id"])
        for row in topology["compute_app_mappings"]
    ]
    current_identity = [
        (row["host_pid"], row["gpu_uuid"], row["pci_bus_id"])
        for row in current_topology["compute_app_mappings"]
    ]
    if (
        topology["process_group_id"] != current_topology["process_group_id"]
        or topology["mapped_gpu_uuids"] != current_topology["mapped_gpu_uuids"]
        or topology["prelaunch_compute_apps_sha256"]
        != current_topology["prelaunch_compute_apps_sha256"]
        or topology_identity != current_identity
    ):
        raise ValueError("vLLM process-group/GPU ownership changed before validation")
...
def execute_final_pipeline(
    ...
    gpus = _gpu_identities(environment)
    prelaunch_compute_apps = capture_empty_compute_baseline(environment, gpus)

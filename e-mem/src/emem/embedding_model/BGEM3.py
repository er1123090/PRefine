...
def cls_pooling(token_embeddings):
    """Extract CLS token embedding (first token) from the output."""
...
class BGEM3EmbeddingModel(BaseEmbeddingModel):
    ...
    def _add_instruction_prefix(self, texts: List[str], instruction: str = "") -> List[str]:
        """Add instruction prefix to texts for BGE-M3 models if needed."""
        if instruction:
    ...
    def encode(self, texts: List[str], instruction: str = ""):
        ...
        texts = self._add_instruction_prefix(texts, instruction)
        ...
        with torch.no_grad():
            inputs = self.tokenizer(
                ...
                padding=True, 
                truncation=True, 
                max_length=self.embedding_config.encode_params.get("max_length", 512),
                return_tensors='pt'
            ).to(self.embedding_model.device)
            
            outputs = self.embedding_model(**inputs)
            # BGE-M3 uses CLS token pooling (first token of last hidden state)
            embeddings = cls_pooling(outputs[0])

        return embeddings

    def batch_encode(self, texts: List[str], **kwargs) -> np.ndarray:
        """Batch encode texts and return embeddings as numpy array."""
        ...
        if len(texts) <= batch_size:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                results.append(self.encode(batch_texts, instruction=instruction))

#!/bin/bash
# Self-Refine singleturn inference via vLLM (OpenAI-compatible endpoint).
...
echo "########################################################################"
echo "Self-Refine vLLM Singleturn Inference"
echo "Start: $(date)"
...
for model in "${MODELS[@]}"; do
    ...
    for pref in "${PREF_TYPES[@]}"; do
        ...
        DATE_TAG="$(date +%m%d)"
        MODEL_SAFE="${model//\//_}"
        OUT_DIR="$ROOT_DIR/outputs/self_refine/vllm/singleturn/$MODEL_SAFE/$pref"
        LOG_DIR="$ROOT_DIR/logs/self_refine/vllm/singleturn/$MODEL_SAFE/$pref"

...
SOURCE_DIR = "/data/minseo/experiments4/mem0"
...
from common_fixed_pairs import (
    ...
    build_multiturn_ground_truth,
    get_multiturn_utterance,
    get_sub_idx,
    load_fixed_pairs_items,
    load_multiturn_template_lookup,
    load_python_module,
)


SOURCE_MODULE_PATH = f"{SOURCE_DIR}/step2_evaluate_multiturn.py"
source = load_python_module(
    SOURCE_MODULE_PATH,
    "extended_schema_mem0_multi_source",
    extra_sys_path=SOURCE_DIR,
)


EXTENDED_INPUT_PATH = "/data/minseo/experiments4/data/1229_dev_6.json"
EXTENDED_MULTITURN_PATH = "/data/minseo/experiments4/query_new_multi.json"
EXTENDED_PREF_LIST_PATH = "/data/minseo/experiments4/pref_list_extended.json"
EXTENDED_PREF_GROUP_PATH = "/data/minseo/experiments4/pref_group_extended.json"
EXTENDED_SCHEMA_PATH = "/data/minseo/experiments4/schema_all_extended_complete.json"
...
def build_fixed_prepared_items(
    input_path: str,
    multiturn_path: str,
    fixed_pairs_path: str,
) -> List[Dict[str, Any]]:
    df = source.load_chains_dataset(input_path)
    fixed_items = load_fixed_pairs_items(fixed_pairs_path)
    template_lookup = load_multiturn_template_lookup(multiturn_path)
        ...
        prepared_items.append(
            {
                ...
                "utterance": get_multiturn_utterance(source, item),
                "ground_truth": build_multiturn_ground_truth(source, template_lookup, item),
...
async def process_with_llm_async(
    ...
    if not fixed_pairs_path and max_queries is None:
    ...
    if fixed_pairs_path:
        prepared_items = build_fixed_prepared_items(input_path, multiturn_path, fixed_pairs_path)
        if max_queries is not None:
            print("[Info] fixed_pairs_path is active; ignoring max_queries sampling.")
    else:
        multiturn_data = source.load_multiturn_data(multiturn_path)
        prepared_items = []
        skipped_count = 0
...
def main() -> None:
    ...
    parser.add_argument("--multiturn_path", type=str, default=EXTENDED_MULTITURN_PATH)

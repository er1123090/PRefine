...
pip install -r /data/minseo/experiments6/langmem/requirements.txt
```

## Python 3.10 note

`langmem==0.0.30` imports `typing.NotRequired` directly, which breaks on Python 3.10. The scripts in this folder include a compatibility shim in [`_compat.py`](/data/minseo/experiments6/langmem/_compat.py) so they can still import the runtime on Python 3.10.
...
```bash
python /data/minseo/experiments6/langmem/step1_build_memory.py \
  --memory_model gpt-4o-mini \
  --output_path /data/minseo/experiments6/langmem/memory_snapshots/langmem_1229_dev_6.jsonl \
  --manifest_path /data/minseo/experiments6/langmem/memory_snapshots/langmem_1229_dev_6.manifest.json
```

Build the snapshot with a vLLM-served memory model:

```bash
bash /data/minseo/experiments6/langmem/run_langmem_build_vllm.sh
```
...
```bash
python /data/minseo/experiments6/langmem/langmem_inference_single.py \
  --memory_path /data/minseo/experiments6/langmem/memory_snapshots/langmem_1229_dev_6.jsonl \
  --pref_type easy \
...
```bash
python /data/minseo/experiments6/langmem/langmem_inference_multi.py \
  --memory_path /data/minseo/experiments6/langmem/memory_snapshots/langmem_1229_dev_6.jsonl \
  --pref_type easy \
  --context_type memory_only \
  --model_name gpt-5
...
```bash
python /data/minseo/experiments6/langmem/measure_session_memory_token_delta.py \
  --memory_model gpt-4o-mini \
  --output_csv /data/minseo/experiments6/langmem/session_memory_token_deltas_1229_dev_6.csv \
  --summary_json /data/minseo/experiments6/langmem/session_memory_token_deltas_1229_dev_6.summary.json
```

Plot memory growth:

```bash
python /data/minseo/experiments6/langmem/plot_session_memory_token_growth.py \
  --input_csv /data/minseo/experiments6/langmem/session_memory_token_deltas_1229_dev_6.csv
```

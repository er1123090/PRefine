...
def get_support_json_schema():
    ...
    if env_value is None:
        logger.warning("Environment variable SUPPORT_JSON_SCHEMA is not set. Defaulting to true.")
        return True
...
class BaseConfig:
    ...
    api_key: str = field(
        default=None,
        metadata={"help": "API key for the LLM model. If unset, the runtime falls back to OPENAI_API_KEY."}
    )
    ...
    embedding_api_key: str = field(
        default=None,
        metadata={"help": "API key for the embedding model. If unset, the runtime falls back to api_key or OPENAI_API_KEY."}
    )

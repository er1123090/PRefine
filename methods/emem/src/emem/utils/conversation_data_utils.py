...
class QA:
    ...
    def final_answer(self) -> Optional[str]:
        """Get the appropriate ground truth answer based on category."""
        if self.category == 5:
...
class Turn:
    speaker: str
    dia_id: str # in the format of "D{session idx}:{turn idx}". Note turn idx also starts from 1
    text: str # if image exists, this is the concatenation of image BLIP caption and the utterance text

@dataclass
class Session:
    session_id: int # idx from 1
    date_time: str # e.g., "1:56 pm on 8 May, 2023"
    turns: List[Turn]

@dataclass
class Conversation:
    speaker_a: str # name of speaker
    speaker_b: str
    sessions: Dict[int, Session] # from session idx (starting from 0) to parsed session

@dataclass
class EventSummary:
    # from "events_session_{session idx}" to {speaker_a_name: List of event summary strings, speaker_b_name: list of event summary strings, "date": date}
    events: Dict[str, Dict[str, List[str]]]  # session -> speaker -> events

@dataclass
class Observation:
    # from "session_{session_idx}_observation" to {speaker_a_name: List of [observation string, {dia_id} for the turn], speaker_b_name: List of [observation string, {dia_id} for the turn]}
    observations: Dict[str, Dict[str, List[List[str]]]]  # session -> speaker -> [observation, evidence]

@dataclass
class LoCoMoSample:
    """A single sample from the LoComo dataset"""
    sample_id: str # self-assigned relative idx starting from 0
    qa: List[QA]
    conversation: Conversation
    event_summary: EventSummary
    observation: Observation
    session_summary: Dict[str, str] # from "session_{session idx}_summary" to the summary string
...
def parse_conversation(conv_data: dict) -> Conversation:
    ...
    sessions: Dict[int, Session] = {} # from int (session id) to corresponding session
            ...
            for qa_idx, qa in enumerate(sample["qa"]):
                ...
                try:
                    ...
                    for evidence_id in qa["evidence"]:
                        ...
                        #     continue
                        session_id: str = evidence_id.split(":")[0].strip()[1:]
                        turn_id: str = evidence_id.split(":")[1].strip()
                        evidence_turn = sample["conversation"][f"session_{session_id}"][int(turn_id) - 1]
                        assert evidence_turn["dia_id"] == evidence_id
                        if "img_url" in evidence_turn or "blip_caption" in evidence_turn:
                    ...
                    if evidence_id in ["D:11:26", ]: continue
                    print("Error evidence id: ", evidence_id)
                    print(f"Error in sample {sample_idx}, QA pair {qa_idx}:")
                    print(f"QA data: {qa}")
                    raise e
                except Exception as e:
                    if evidence_id in ["D8:6; D9:17", "D10:19", 'D', "D4:36", "D9:1 D4:4 D4:6", "D22:1 D22:2 D9:10 D9:11", "D21:18 D21:22 D11:15 D11:19", "D30:05"]: continue
                    print("Error evidence id: ", evidence_id)
...
if __name__ == "__main__":
    # Example usage
    dataset_path = Path(__file__).parent.parent.parent.parent / "data" / "locomo10.json"
    try:

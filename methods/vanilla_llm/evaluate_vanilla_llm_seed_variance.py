#!/usr/bin/env python3
"""Evaluate vanilla_llm seed-variance inference outputs."""
...
def result_path(run_root: Path, repeat: str, turn: str, model: str, difficulty: str) -> Path:
    return (
        run_root
        / "repeats"
        / repeat
...
def write_seed_outputs(out_root: Path, per_file: List[Dict[str, Any]], per_example: List[Dict[str, Any]], coverage: List[Dict[str, Any]]) -> None:
    ...
    for repeat in sorted({row["repeat_id"] for row in per_file}):
        ...
        write_summary(seed_root / "summary_by_model_turn_difficulty.csv", seed_files, ("inference_model", "turn", "difficulty"))
...
def main() -> None:
    ...
    run_root = ROOT / "outputs" / "vanilla_llm" / args.parent_run_id
    out_root = ROOT / "results" / "vanilla_llm" / args.parent_run_id
    metadata_path = run_root / "metadata.json"
    metadata = json.loads(metadata_path.read_text(encoding="utf-8")) if metadata_path.exists() else {}
    inference_models = metadata.get(
        "inference_models",
        [
    ...
    repeats = metadata.get("repeats", ["seed_001", "seed_002", "seed_003"])
        ...
        for model in inference_models:
            for turn in turns:
                for difficulty in difficulties:
                    path = result_path(run_root, repeat, turn, model, difficulty)
                    base = {
                        "repeat_id": repeat,
                        "turn": turn,
                        "difficulty": difficulty,
                        "inference_model": model_safe(model),
                        "canonical_inference_model_id": model,
                        "method": "vanilla_llm",
                        "context": CONTEXT,
                        "prompt_type": PROMPT,
                        "file": str(path),
                        ...
                        per_example.append(
                            {
                                **base,
                                ...
                                "pair_key": f"{turn}|{difficulty}|{repeat}|{example_id_sub}",
                                **counts,
                            }
                        )
    ...
    write_summary(out_root / "summary_by_model_turn_difficulty.csv", per_file, ("inference_model", "turn", "difficulty"))

...
PYTHON_SCRIPT="$RELEASE_ROOT/extended_schema/RAG/RAG_inference_single_fixed_pairs.py"

INPUT_PATH="$RELEASE_ROOT/data/1229_dev_6.json"
QUERY_PATH="$RELEASE_ROOT/query_new_single.json"
FIXED_PAIRS_PATH="$RELEASE_ROOT/data/fixed_singleturn_example_query_pairs_400.json"
PREF_LIST_PATH="$RELEASE_ROOT/pref_list_extended.json"
PREF_GROUP_PATH="$RELEASE_ROOT/pref_group_extended.json"
TOOLS_SCHEMA_PATH="$RELEASE_ROOT/schema_easy_extended.json"
DB_PATH="${DB_PATH:-$RELEASE_ROOT/RAG/chroma_db_rag}"
COLLECTION_NAME="${COLLECTION_NAME:-user_memories}"

RUN_ROOT="$RELEASE_ROOT/extended_schema/RAG/output"
DATE_TAG="$(date +%m%d)"
...
echo "========================================================"
echo "RAG Fixed-400 Single-turn Started at $(date)"
...
for model in "${MODELS[@]}"; do
    ...
    BASE_OUTPUT_DIR="$RUN_ROOT/singleturn/$BACKEND_TAG"
    BASE_LOG_DIR="$RUN_ROOT/logs/singleturn/$BACKEND_TAG"
    mkdir -p "$BASE_OUTPUT_DIR" "$BASE_LOG_DIR"

    for context in "${CONTEXT_TYPES[@]}"; do
        for pref in "${PREF_TYPES[@]}"; do
            CURRENT_OUT_DIR="$BASE_OUTPUT_DIR/$context/$pref/singleturn-query/${MODEL_SAFE_NAME}_${EFFORT_TAG}/$PROMPT_NAME"
            CURRENT_LOG_DIR="$BASE_LOG_DIR/$context/$pref/singleturn-query/${MODEL_SAFE_NAME}_${EFFORT_TAG}/$PROMPT_NAME"
...
echo "========================================================"
echo "RAG Fixed-400 Single-turn Finished at $(date)"
echo "========================================================"

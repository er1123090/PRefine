#!/usr/bin/env python3
"""Evaluate MEMORY3 context-ablation vLLM outputs."""
...
def write_markdown_summary(path: Path, rows: List[Dict[str, Any]]) -> None:
    ordered = sorted(rows, key=lambda r: (r["input_context"], r["inference_model"]))
    lines = [
        "# MEMORY3 context ablation summary",
        "",
        "| input_context | inference_model | n_files | n_examples | precision | recall | f1 | pref_em | nonpref_f1 | parse_fail |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in ordered:
        lines.append(
            "| {input_context} | {inference_model} | {n_files} | {n_examples} | "
            "{overall_precision:.4f} | {overall_recall:.4f} | {overall_f1:.4f} | "
            "{pref_em:.4f} | {nonpref_f1:.4f} | {parse_failure_rate:.4f} |".format(**row)
        )
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    inference_models = metadata.get("inference_models", [])
    memory_sources = metadata.get("memory_sources", [])
    repeats = metadata.get("repeats", ["seed_001"])
    turns = metadata.get("turns", ["singleturn", "multiturn"])
    difficulties = metadata.get("difficulties", ["easy", "medium", "hard"])
    contexts = metadata.get(
        "contexts",
        ["memory_only", "full_dialogue_only", "api_only", "memory_api", "full_dialogue_api"],
    )
    context_memory_sources = metadata.get("context_memory_sources") or {}
    for context in contexts:
        context_memory_sources.setdefault(
            context,
            ["none"] if context == "full_dialogue_only" else memory_sources,
        )
    ...
    for repeat in repeats:
        for inference_model in inference_models:
            for context in contexts:
                for memory_safe in context_memory_sources.get(context, []):
                    for turn in turns:
                        for difficulty in difficulties:
                            ...
                            base = {
                                ...
                                "memory_method": "none" if memory_safe == "none" else "ours_memory_v1",
                                ...
                                per_example.append(
                                    {
                                        **base,
                                        ...
                                        "pair_key": (
                                            f"{turn}|{difficulty}|{repeat}|{context}|"
                                            f"{memory_safe}|{example_id_sub}"
                                        ),
                                        **counts,
                                    }
                                )
    ...
    write_summary(out_root / "summary_by_context.csv", per_file, ("input_context",))
    write_summary(out_root / "summary_by_context_model.csv", per_file, ("input_context", "inference_model"))
    write_summary(
        out_root / "summary_by_context_model_turn_difficulty.csv",
        per_file,
        ("input_context", "inference_model", "turn", "difficulty"),
    )
    write_summary(
        out_root / "summary_by_context_memory_model.csv",
        per_file,
        ("input_context", "memory_safe_name", "inference_model"),
    )
    write_summary(
        out_root / "summary_by_context_memory_model_turn_difficulty.csv",
        per_file,
        ("input_context", "memory_safe_name", "inference_model", "turn", "difficulty"),
    )

    context_model_rows_path = out_root / "summary_by_context_model.csv"
    if context_model_rows_path.exists() and per_file:
        grouped: Dict[Tuple[Any, ...], List[Dict[str, Any]]] = defaultdict(list)
        for row in per_file:
            grouped[(row["input_context"], row["inference_model"])].append(row)
        context_model_rows = [
            aggregate(group_rows, ("input_context", "inference_model"))
            for group_rows in grouped.values()
        ]
        write_markdown_summary(out_root / "summary_by_context_model.md", context_model_rows)

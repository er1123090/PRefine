...
def load_multiturn_data(fpath: str) -> Dict[str, Any]:
    ...
    Supports either:
      1. { "GetRestaurants": [ { "query": [...], "api_call": [...] } ] }
      2. [ { "query_id": "...", "target": [{"domain": "GetRestaurants", ...}], ... } ]
    """
    ...
    if isinstance(data, list):
        ...
        for item in data:
            ...
            if not domain:
                api_calls = item.get("api_call", [])
                if api_calls and isinstance(api_calls, list):
                    first_api_call = api_calls[0]
                    if isinstance(first_api_call, str) and "(" in first_api_call:
                        domain = first_api_call.split("(", 1)[0].strip()
    ...
    print(f"[Error] Unsupported multi-turn query format: {type(data).__name__}")
    return {}
...
def get_multiturn_target_slots(template_data: Dict[str, Any]) -> set:
    slots = set()
    for target in template_data.get("target", []):
...
def select_multiturn_template(
    ...
    extracted_pref_map: Dict[str, List[str]]
) -> Optional[Dict[str, Any]]:
    if not templates:
        return None

    target_slots = set(extracted_pref_map.keys())
    if not target_slots:
        return templates[0]
    ...
    for template_data in templates:
        overlap_score = len(target_slots & get_multiturn_target_slots(template_data))
...
def assign_user_utterances(
    ...
    def process_extraction(domain, extracted_pref_map):
        raw_templates = multiturn_data.get(domain, [])
        if not raw_templates:
            return None

        templates = raw_templates if isinstance(raw_templates, list) else [raw_templates]
        template_data = select_multiturn_template(templates, extracted_pref_map)
...
# ---------------------------------------------------------
# 5. Async Processing Pipeline (Updated with Parsing)
# ---------------------------------------------------------
async def process_single_item(
    ...
    async with semaphore:
        ...
        # 1. Get Raw Output from LLM
        raw_output_text = await call_llm_api_async(
            prompt, model_name, openai_client, tools_schema, reasoning_effort
        )

        # 2. [NEW] Parse Reasoning & Clean Output
        parsed = parse_reasoning(raw_output_text)
        clean_llm_output = parsed["llm_output"]
        reasoning_content = parsed["reasoning_content"]

        # 3. Logging (saving separate fields)
        log_record = {
            ...
            "model_input": prompt,
            "model_output": clean_llm_output,         # Clean output in main field
            "reasoning_content": reasoning_content,   # Separate reasoning field
            "raw_content": raw_output_text            # Backup full raw content
        }
        ...
        # Save parsed fields to result
        result_ex["llm_output"] = clean_llm_output
        result_ex["reasoning_content"] = reasoning_content
        result_ex["raw_content"] = raw_output_text
...
if __name__ == "__main__":
    ...
    parser.add_argument("--multiturn_path", type=str, default="/data/minseo/experiments4/query_multiturn-domain.json", help="Path to multi-turn query json") 
    ...
    parser.add_argument("--tools_schema_path", type=str, default="/data/minseo/experiments4/schema_all.json")

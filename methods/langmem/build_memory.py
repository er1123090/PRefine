...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a LangMem semantic-memory snapshot from the experiments4 dataset."
    )
    parser.add_argument("--input_path", type=str, default=DEFAULT_INPUT_PATH)
    parser.add_argument(
        "--output_path",
        type=str,
        default="/data/minseo/experiment8/outputs/langmem/memory.jsonl",
    )
    parser.add_argument(
        "--manifest_path",
        type=str,
        default="/data/minseo/experiment8/outputs/langmem/memory.manifest.json",
    )
...
async def main() -> None:
    ...
    setup_usage_token = begin_usage_collection()
    try:
        ...
        setup_token_usage = end_usage_collection(setup_usage_token)
    logger.info(
    ...
    async def process_example(example: Dict[str, Any]) -> None:
        async with semaphore:
            ...
            try:
                for session_index, session in enumerate(example.get("sessions", []), start=1):
                    ...
                    if memory_backend == "structured_output":
                        ...
                        manager_config["callbacks"] = [construction_callback]
                        await runtime.manager.ainvoke(
                            {"messages": messages}, config=manager_config
                        )
                    ...
                    session_usage = current_usage_report()["by_session"].get(
                        str(session_index)
                    )
                    session_exports.append(
                        {
                            ...
                            "construction_token_usage": session_usage,
                        }
                    )
            finally:
            ...
            record["method"] = "langmem"
            record["memory_mode"] = memory_backend
            record["token_encoding"] = "cl100k_base"
            record["construction_token_usage"] = construction_token_usage
            record["token_counts"] = construction_token_usage["summary"]
    ...
    manifest = {
        ...
        "setup_token_usage": setup_token_usage,
    }

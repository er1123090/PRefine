...
PYTHON_SCRIPT="/data/minseo/experiments5/methods/langmem/langmem_inference_multi.py"
BASE_DIR="/data/minseo/experiments5"
INPUT_PATH="${BASE_DIR}/data/1229_dev_6.json"
MULTITURN_PATH="${BASE_DIR}/config/query_multiturn.json"
...
MEMORY_ROOT="${MEMORY_ROOT:-${BASE_DIR}/methods/langmem/memory_snapshots/semantic-custom}"
OUTPUT_ROOT="${BASE_DIR}/methods/langmem/inference_multi_low"
...
LOCK_DIR="${OUTPUT_ROOT}/.run_langmem_multi.lock"
...
RESUME_COMPLETED_RUNS="${RESUME_COMPLETED_RUNS:-1}"
...
acquire_lock() {
    local existing_pid=""
    local other_runner_pid=""

    mkdir -p "${OUTPUT_ROOT}"

    other_runner_pid="$(
        ps -eo pid=,args= | awk -v self="$$" '
            $1 != self && $0 ~ /(^|[[:space:]])bash([[:space:]].*)?run_langmem_multi\.sh([[:space:]]|$)/ {
                print $1
                exit
            }
        '
    )"
    if [ -n "${other_runner_pid}" ] && ps -p "${other_runner_pid}" > /dev/null 2>&1; then
        echo "[ERROR] Another run_langmem_multi.sh process is already running (PID: ${other_runner_pid})."
    ...
    if [ -n "${existing_pid}" ] && ps -p "${existing_pid}" > /dev/null 2>&1; then
        echo "[ERROR] Another run_langmem_multi.sh instance is already running (PID: ${existing_pid})."
        echo "        Stop the existing run or wait for it to finish before starting a new one."
        exit 1
    fi

    echo "[WARN] Found a stale multiturn lock. Replacing it."
    rm -rf "${LOCK_DIR}"
    if mkdir "${LOCK_DIR}" 2>/dev/null; then
        printf '%s\n' "$$" > "${LOCK_PID_FILE}"
        LOCK_HELD=1
    ...
    echo "[ERROR] Failed to acquire multiturn runner lock at ${LOCK_DIR}."
    exit 1
}
...
resume_completed_runs_enabled() {
    case "${RESUME_COMPLETED_RUNS,,}" in
        1|true|yes|y|on)
            return 0
            ;;
        *)
...
latest_completed_run_log() {
    local run_dir="$1"
    local run_log=""
    local finish_line=""
    local output_path=""
    local structured_log_path=""

    [ -d "${run_dir}" ] || return 1

    while IFS= read -r run_log; do
        [ -n "${run_log}" ] || continue
        finish_line="$(grep -F "Inference finished:" "${run_log}" | tail -n 1 || true)"
        [ -n "${finish_line}" ] || continue

        output_path="$(printf '%s\n' "${finish_line}" | sed -n 's/.*output=\([^ ]*\) structured_log=.*/\1/p')"
        structured_log_path="$(printf '%s\n' "${finish_line}" | sed -n 's/.* structured_log=\([^ ]*\)$/\1/p')"

        if [ -n "${output_path}" ] && [ -f "${output_path}" ] && [ -n "${structured_log_path}" ] && [ -f "${structured_log_path}" ]; then
            printf '%s\n' "${run_log}"
            return 0
        fi
    done < <(find "${run_dir}" -maxdepth 1 -type f -name '*.run.log' | sort -r)

    return 1
}

model_has_pending_work() {
    local model_safe_name="$1"
    local target_memory_folder="${2:-}"
    local memory_folder=""
    local context=""
    local pref=""
    local run_dir=""

    if ! resume_completed_runs_enabled; then
        return 0
    fi

    for memory_folder in "${MEMORY_FOLDERS[@]}"; do
        if [ -n "${target_memory_folder}" ] && [ "${memory_folder}" != "${target_memory_folder}" ]; then
            continue
        fi
        for context in "${CONTEXT_TYPES[@]}"; do
            for pref in "${PREF_TYPES[@]}"; do
                run_dir="${OUTPUT_ROOT}/${memory_folder}/${context}/${pref}/${model_safe_name}/${PROMPT_NAME}"
                if ! latest_completed_run_log "${run_dir}" > /dev/null; then
                    return 0
                fi
            done
        done
...
memory_has_pending_work() {
    local target_memory_folder="$1"
    local model=""
    local model_safe_name=""

    for model in "${MODELS[@]}"; do
        model_safe_name="${model//\//_}"
        if model_has_pending_work "${model_safe_name}" "${target_memory_folder}"; then
            return 0
        fi
    done
...
echo "========================================================"
echo "LangMem Multiturn Inference Started at $(date)"
echo "GPU: ${GPU_IDS} | TP Size: ${TP_SIZE}"
echo "Models: ${MODELS[*]}"
echo "Resume Completed Runs: ${RESUME_COMPLETED_RUNS}"
if [ "${#MEMORY_MODELS[@]}" -gt 0 ]; then
...
for memory_folder in "${MEMORY_FOLDERS[@]}"; do
    if ! memory_has_pending_work "${memory_folder}"; then
        echo "####################################################################"
        echo "[SKIP] All requested multiturn runs already exist for memory: ${memory_folder}"
        echo "####################################################################"
        continue
    fi
    ...
    job_specs_path="$(mktemp "${OUTPUT_ROOT}/.${memory_folder}.multi.batch.XXXXXX.jsonl")"
    ...
    for model in "${MODELS[@]}"; do
        ...
        if ! model_has_pending_work "${model_safe_name}" "${memory_folder}"; then
            echo "####################################################################"
            echo "[SKIP] All requested multiturn runs already exist for: ${memory_folder} / ${model}"
            echo "####################################################################"
            continue
        fi
        ...
        echo "[STEP 3] Queueing Multiturn Experiments for Current Memory..."
        ...
        for context in "${CONTEXT_TYPES[@]}"; do
            ...
            for pref in "${PREF_TYPES[@]}"; do
                run_dir="${OUTPUT_ROOT}/${memory_folder}/${context}/${pref}/${model_safe_name}/${PROMPT_NAME}"
                if resume_completed_runs_enabled && completed_run_log="$(latest_completed_run_log "${run_dir}")"; then
                    skipped_pref_types+=("${pref}")
                    echo " >> [SKIP] Completed: ${memory_folder} / ${context} / ${pref} / ${model_safe_name} ($(basename "${completed_run_log}"))"
                    continue
                fi
                pending_pref_types+=("${pref}")
            done

            if [ "${#pending_pref_types[@]}" -eq 0 ]; then
                echo " >> [SKIP] All requested prefs already complete for ${memory_folder} / ${context} / ${model_safe_name}"
                continue
            fi
            ...
            echo "    - Context/Prefs   : ${context} / ${pending_pref_types[*]}"
            if [ "${#skipped_pref_types[@]}" -gt 0 ]; then
                echo "    - Skipping Prefs  : ${skipped_pref_types[*]}"
            fi
            echo "    - Memory Path     : ${memory_path}"
            echo "    - Embedding Model : ${embedding_model}"
...
echo "========================================================"
echo "All LangMem Multiturn Jobs Finished at $(date)"
echo "========================================================"
cleanup_temp_job_specs
release_lock

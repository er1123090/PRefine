...
# Run the conflict-majority memory/inference matrix from ivy while freesia
# serves one vLLM model at a time on GPUs 2,3.

set -u
set -o pipefail

ROOT_DIR="${ROOT_DIR:-/data/minseo/experiments5}"
RUN_ID="${RUN_ID:-conflict_majority_freesia23_$(date +%Y%m%d_%H%M%S)}"
...
REMOTE_LOG_DIR="/data/minseo/.logs/experiments5_conflict_majority_freesia23_$RUN_ID"
...
MATRIX_RUNNER="$ROOT_DIR/scripts/run_conflict_majority_memory_matrix_vllm.sh"
...
MEMORY_STAGE_MODELS=("${MEMORY_SOURCE_MODELS[@]}")
if [[ -n "${ONLY_MEMORY_STAGE_MODELS:-}" ]]; then
  read -r -a MEMORY_STAGE_MODELS <<< "$ONLY_MEMORY_STAGE_MODELS"
fi
...
main() {
  ...
  log "Memory models: ${MEMORY_SOURCE_MODELS[*]}"
  log "Memory stage models: ${MEMORY_STAGE_MODELS[*]}"
  log "Inference models: ${INFERENCE_MODELS[*]}"

  start_tunnel

  log "Building conflict task bundles"
  run_matrix_stage tasks env

  local model
  for model in "${MEMORY_STAGE_MODELS[@]}"; do
    if start_model "$model"; then
      log "Running memory stage for $model"
      run_matrix_stage memory env ONLY_MEMORY_MODELS="$model"
    else
      log "Failed to start memory model=$model; continuing"
    fi
  done

  for model in "${INFERENCE_MODELS[@]}"; do
    if start_model "$model"; then
      log "Running inference stage for $model"
      run_matrix_stage inference env ONLY_MODELS="$model"
    else
  ...
  run_matrix_stage eval env
  log "Done. Outputs: $ROOT_DIR/outputs/our_memory/$RUN_ID"
  log "Done. Results: $ROOT_DIR/results/our_memory/$RUN_ID"
}
